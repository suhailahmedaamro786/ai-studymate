...
async def process_document(document_id: str, owner_id: str, storage_path: str):
    ...
    db.table("documents").update({"status": "processing", "error_message": None}).eq("id", document_id).eq("owner_id", owner_id).execute()
    ...
    try:
        ...
        if not valid_chunks:
            raise ValueError("No embeddings were generated. Configure OPENAI_API_KEY for the RAG embedding pipeline.")

        # Safe retry behavior: processing the same document twice must not duplicate chunks.
        db.table("document_chunks").delete().eq("document_id", document_id).eq("owner_id", owner_id).execute()
        ...
        logger.info("Document %s processed: %s chunks from %s pages", document_id, len(valid_chunks), page_count)
    except Exception as exc:
        logger.exception("Document processing failed for %s", document_id)
        db.table("documents").update({"status": "failed", "error_message": str(exc)[:500]}).eq("id", document_id).eq("owner_id", owner_id).execute()
