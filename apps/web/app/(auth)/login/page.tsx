...
export default function LoginPage() {
  ...
  const handleOAuth = async (provider: Provider) => {
    ...
    try {
      ...
      if (error) {
        setError(`Unable to sign in with ${provider === "google" ? "Google" : provider === "facebook" ? "Facebook" : "GitHub"}. Please try again.`);
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl shadow-2xl shadow-primary/10 p-8 space-y-6">
          <div className="flex flex-col items-center text-center space-y-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">Continue your personalized learning journey.</p>
            </div>
          </div>
          ...
          <div className="space-y-3">
            {PROVIDERS.map(({ key, label, icon }) => (
              ...
              >
                ...
                {loadingProvider === key ? "Signing in..." : label}
              </Button>
            ))}
          </div>
